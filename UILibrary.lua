...
local DevilHubFolder = "DevilHub"
local ConfigurationFolder = DevilHubFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local DevilHubQuality = {}

local DevilHubLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across DevilHub
		...
		Light = {
			TextFont = "Gotham",  -- Default will use the various font faces used across DevilHub
...
local DevilHub = game:GetObjects("rbxassetid://13853811008")[1]
DevilHub.Enabled = false
local spawn = task.spawn
local delay = task.delay

--Studio
if game["Run Service"]:IsStudio() then
	function gethui() return DevilHub end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end
end

pcall(function()
	_G.LastRayField.Name = 'Old DevilHub'
...
local ParentObject = function(Gui)
	...
	_G.LastRayField = DevilHub
end
ParentObject(DevilHub)

--Object Variables

local Camera = workspace.CurrentCamera
local Main = DevilHub.Main
...
local InfoPrompt = DevilHub.Info

DevilHub.DisplayOrder = 100
...
local BarType = 'Side'
local HoverTime = 0.3
local Notifications = DevilHub.Notifications

local SelectedTheme = DevilHubLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = DevilHub.Theme[ThemeName]
	for _, obj in ipairs(DevilHub:GetDescendants()) do
	...
	DevilHub.Main.BackgroundColor3 = SelectedTheme.Background
	DevilHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	DevilHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	DevilHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	DevilHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	DevilHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	DevilHub.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if DevilHubLibrary.Flags[FlagName] then
			spawn(function() 
				if DevilHubLibrary.Flags[FlagName].Type == "ColorPicker" then
					DevilHubLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if DevilHubLibrary.Flags[FlagName].CurrentValue or DevilHubLibrary.Flags[FlagName].CurrentKeybind or DevilHubLibrary.Flags[FlagName].CurrentOption or DevilHubLibrary.Flags[FlagName].Color ~= FlagValue then DevilHubLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			DevilHubLibrary:Notify({Title = "Flag Error", Content = "DevilHub was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(DevilHubLibrary.Flags) do
...
function DevilHubLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= DevilHubLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("DevilHub | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if NotificationSettings.Image then
			...
			Notification.Icon.Image = "rbxassetid://12416116403"
		end
		...
		if DevilHub.Name == "DevilHub" then
...
function Hide()
	...
	Debounce = true
	DevilHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function DevilHubLibrary:CreateWindow(Settings)
	DevilHub.Enabled = false
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "DevilHub Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius | Meta"
	if Settings.LoadingTitle ~= "DevilHub Interface Suite" then
		LoadingFrame.Version.Text = "DevilHub UI"
	end
	...
	pcall(function()
		...
		if not isfolder(DevilHubLibrary.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(DevilHubFolder.."/Discord Invites") then
			makefolder(DevilHubFolder.."/Discord Invites")
		end
		if not isfile(DevilHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(DevilHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"DevilHub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(DevilHubFolder.."/Key System") then
			makefolder(DevilHubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("DevilHub | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(DevilHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(DevilHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function(EnterPressed)
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(DevilHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						DevilHubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
	...
	--	qNotePrompt({
	--		Title = 'Preview',
	--		Description = 'This is a preview for the official DevilHub forum post. Remember that things are subject to change.',
	...
	DevilHubQuality.Window = {Tabs = {}}
	local Window = DevilHubQuality.Window
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("DevilHub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateSection(SectionName,Display)
			...
			if Display then
				...
				SectionValue.Holder.Parent = DevilHub.Holding
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function(enter)
				...
				if not Success then
					...
					print("DevilHub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			local function AddOption(Option,Selecteds)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("DevilHub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
					end
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						DevilHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then  --Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("DevilHub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					DevilHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("DevilHub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					DevilHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					DevilHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			local function UpdateSlider(X)
				...
				if SliderSettings.CurrentValue ~= NewValue then
					...
					if not Success then
						...
						print("DevilHub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					DevilHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			...
			for name,info in pairs(PromptSettings.Actions) do
				...
				Button.Interact.MouseButton1Up:Connect(function()
					if not clicked then
						...
						if not Success then
							ClosePrompt()
							print("DevilHub | "..info.Name.." Callback Error " ..tostring(Response))
						else
							ClosePrompt()
						end
					end
...
function DevilHubLibrary:Destroy()
	DevilHub:Destroy()
end
...
Topbar.Type.MouseButton1Click:Connect(function()
	...
	if SideBarClosed then
		Topbar.Type.Image = "rbxassetid://"..12416116403
		OpenSideBar()
	else
		Topbar.Type.Image = "rbxassetid://"..12416116403
		CloseSideBar()
...
function DevilHubLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				DevilHubLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
task.delay(9, DevilHubLibrary.LoadConfiguration, DevilHubLibrary)
return DevilHubLibrary
